...
public class CodeUtilities implements OpCodeConstants, OpCodeInfo {
	...
	ClassRecord getClassRecord(String className) throws TinyVMException {
		...
		if (ret == null) {
			throw new TinyVMException("Bug CU-3: Didn't find class "
					+ className + " from class " + iCF.getClassName()
					+ " method " + this.iFullName);
		}
		return ret;
	}

	ClassRecord getClassRecord(int aPoolIndex) throws TinyVMException {
		Constant pEntry = iCF.getConstantPool().getConstant(aPoolIndex); // TODO
																			// catch
																			// all
																			// (runtime)
																			// exceptions
	...
	int getConstantIndex(ConstantRecord pRecord) throws TinyVMException {
		...
		if (pIdx == -1) {
			throw new TinyVMException("Bug CU-2: Didn't find constant "
					+ pRecord.toString() + " of class " + iCF.getClassName());
		}
		return pIdx;
	}

	/**
	 * Process a constant index. Given a reference to the constant pool, return
	 * the corresponding index into the leJOS constant table.
	 * 
	 * @param aPoolIndex
	...
	public int processConstantIndex(int aPoolIndex) throws TinyVMException {
		...
		if (!(pEntry instanceof ConstantInteger)
			...
			throw new TinyVMException("Classfile error: LDC-type instruction "
					+ "does not refer to a suitable constant. Class "
					+ iCF.getClassName() + " method " + iFullName);
		}
	...
	/**
	 * Process a class index. Given a constant pool index for a class object,
	 * return the corresponding index into the leJOS class table.
	 * 
	 * @param aPoolIndex
	 *            the constant pool index
	...
	public int processClassIndex(int aPoolIndex) throws TinyVMException {
		...
		if (pIdx == -1) {
			throw new TinyVMException("Bug CU-3: Didn't find class "
					+ pClassRecord.iName + " from class " + iCF.getClassName()
					+ " method " + this.iFullName);
		}
		return pIdx;
	}
	...
	/**
	 * Process and array index. Given an index into the constant pool for an
	 * array class, return the corresponding index into the leJOS class table.
	 * 
	 * @param aPoolIndex
	...
	public int processArray(int aPoolIndex) throws TinyVMException {
		...
		if (pArray == null) {
			throw new TinyVMException(
					"Classfile error: Failed to locate array class for "
							+ pClassRecord.iName + " in class "
							+ iCF.getClassName() + " method " + this.iFullName);
		}
	...
	StaticFieldRecord getStaticFieldRecord(int aFieldIndex)
			throws TinyVMException {
		Constant pEntry = iCF.getConstantPool().getConstant(aFieldIndex); // TODO
																			// catch
																			// all
																			// (runtime)
																			// exceptions
		...
		if (pClassRecord == null) {
			throw new TinyVMException(
					"Attempt to use a field from a primitive array "
							+ className + " from class " + iCF.getClassName()
							+ " method " + iFullName);
		...
		StaticFieldRecord pFieldRecord = pClassRecord
				.getStaticFieldRecord(pName);
		if (pFieldRecord == null) {
			throw new TinyVMException("Failed to locate static field " + pName
					+ " refrenced via class " + className + " from class "
					+ iCF.getClassName());
		}
		return pFieldRecord;
	}
	...
	public void markPrimitiveArray(byte type) throws TinyVMException {
		ClassRecord pClassRecord = getClassRecord(TinyVMType.tinyVMType(type)
				.cname());
		ClassRecord pArray = iBinary.getClassRecordForArray(pClassRecord);
		if (pArray == null) {
			throw new TinyVMException(
	...
	void markStaticField(int aFieldIndex) throws TinyVMException {
		...
		StaticFieldRecord pFieldRecord = pClassRecord
				.getStaticFieldRecord(pName);
		if (pFieldRecord == null) {
			throw new TinyVMException("Failed to mark/locate static field "
					+ pName + " refrenced via class " + className
	 ...
	 * 
	 * @param aFieldIndex
	 *            The index in the constant pool of the static field
	 ...
	 * 
	 * @param aFieldIndex
	 *            The constant pool index for the field.
	 * @throws js.tinyvm.TinyVMException
	 */
	...
	int processMethod(int aMethodIndex, boolean aSpecial, boolean aInterface)
		...
		if (aSpecial) {
			...
			assert pClassIndex != -1
					&& pClassIndex < TinyVMConstants.MAX_CLASSES : "Check: class index in range";
	...
	MethodRecord findMethod(int aMethodIndex, boolean aSpecial,
			boolean aInterface) throws TinyVMException {
		Constant pEntry = iCF.getConstantPool().getConstant(aMethodIndex); // TODO
																			// catch
																			// all
																			// (runtime)
																			// exceptions
		...
		Signature pSig = new Signature(pNT.getName(iCF.getConstantPool()),
				pNT.getSignature(iCF.getConstantPool()));
		...
		ClassRecord pClassRecord = getClassRecord(className);
		if (pClassRecord == null) {
			throw new TinyVMException("Bug CU-7: Didn't find class "
					+ className + " from class " + iCF.getClassName());
		}
		...
		// if (pMethod == null)
		// _logger.log(Level.INFO, "Failed to find " + pSig + " class " +
		// className);
		if (pMethod == null) {
			throw new TinyVMException("Unsupported method:  "
					+ className.replace('/', '.') + '.' + pSig);
		}
		return pMethod;
	}

	/**
	 * Process a constant load operation. Given a reference to the constant
	 * pool, return the corresponding instruction and index required to load it.
	 * 
	 * @param aPoolIndex
	 *            the constant pool index
	...
	public int genConstantLoad(int aPoolIndex) throws TinyVMException {
		...
		if (idx > TinyVMConstants.MAX_CONSTANTS)
			exitOnBadOpCode(OP_LDC);
	 ...
	 * @throws TinyVMException
	 */
	public int genStaticAccess(int aPoolIndex, int optInst, int normInst)
			throws TinyVMException {
		...
		if (valueRec.getAlignment() == 4 && offset / 4 < 256) {
			...
			if (fieldIndex > TinyVMConstants.MAX_STATICS)
				exitOnBadOpCode(optInst);
	...
	/**
	 * Generate an instruction to access an instance field. We use an optimized
	 * version of the opcode if the field is aligned correctly.
	 * 
	 * @param aFieldIndex
	 *            The field we need to access.
	 * @param optInst
	 *            The optimized version of the instruction.
	 * @param normInst
	 *            The normal version of the instruction.
	 ...
	 * @throws TinyVMException
	 */
	int genFieldAccess(int aFieldIndex, int optInst, int normInst)
			throws TinyVMException {
		Constant pEntry = iCF.getConstantPool().getConstant(aFieldIndex); // TODO
																			// catch
																			// all
		...
		int instruction;
		if ((fieldType.type() == TinyVMType.T_INT_TYPE
				|| fieldType.type() == TinyVMType.T_FLOAT_TYPE || fieldType
				.type() == TinyVMType.T_REFERENCE_TYPE) && (pOffset & 0x3) == 0) {
			instruction = optInst;
			iBinary.fieldOpOp++;
		} else {
		...
		return (instruction << 16)
				| (fieldType.type() << TinyVMConstants.F_SIZE_SHIFT) | pOffset;
	}

	static int getAndCopyFourBytesInt(byte[] aCode, int ix, byte[] pOutCode,
			int ox) {
		int a = 0;
	...
	public byte[] processCode(byte[] aCode) throws TinyVMException {
		...
		while (i < aCode.length) {
			...
			i++;
			// System.out.println("Opcode " + OPCODE_NAME[pOpCode] + " addr " +
			// i);
			switch (pOpCode) {
			case OP_LDC: {
			...
			case OP_LDC_W: {
				...
				int inst = genConstantLoad((aCode[i] & 0xFF) << 8
						| (aCode[i + 1] & 0xFF));
				pOutCode[i - 1] = (byte) (inst >> 8);
				pOutCode[i++] = (byte) (inst & 0xFF);
			...
			case OP_GETSTATIC: {
				...
				if (isWrapperTYPEField(idx)) {
					...
					idx = getConstantIndex(getStaticFieldClass(idx)
							.getPrimitiveClass().getClassConstant());
				...
				// fall through
				int inst = genStaticAccess((aCode[i] & 0xFF) << 8
						| (aCode[i + 1] & 0xFF), OP_GETSTATIC, OP_GETSTATIC_1);
			...
			case OP_PUTSTATIC: {
				int inst = genStaticAccess((aCode[i] & 0xFF) << 8
						| (aCode[i + 1] & 0xFF), OP_PUTSTATIC, OP_PUTSTATIC_1);
				pOutCode[i - 1] = (byte) (inst >> 16);
				pOutCode[i++] = (byte) (inst >> 8);
				pOutCode[i++] = (byte) inst;
			}
				break;
			case OP_GETFIELD: {
				int inst = genFieldAccess((aCode[i] & 0xFF) << 8
						| (aCode[i + 1] & 0xFF), OP_GETFIELD, OP_GETFIELD_1);
				pOutCode[i - 1] = (byte) (inst >> 16);
				pOutCode[i++] = (byte) (inst >> 8);
				pOutCode[i++] = (byte) inst;
			}
			...
			case OP_PUTFIELD: {
				int inst = genFieldAccess((aCode[i] & 0xFF) << 8
						| (aCode[i + 1] & 0xFF), OP_PUTFIELD, OP_PUTFIELD_1);
				pOutCode[i - 1] = (byte) (inst >> 16);
				pOutCode[i++] = (byte) (inst >> 8);
				pOutCode[i++] = (byte) inst;
			}
			...
			case OP_INVOKESTATIC:
				// TODO separate invokespecial from invokestatic and check
				// whether method is instance/static
				// Opcode is changed:
			...
			case OP_LOOKUPSWITCH: {
				...
				int dft = getAndCopyFourBytesInt(aCode, i, pOutCode, oi);
				i += 4;
				oi += 4;
				int npairs = getAndCopyFourBytesInt(aCode, i, pOutCode, oi);
				i += 4;
				oi += 4;
				// System.out.println( "lookupswitch: dft: " + dft +
				// ", npairs: " + npairs + ", padding: " + (i - oi));

				for (int k = 0; k < npairs; k++) {
					int idx = getAndCopyFourBytesInt(aCode, i, pOutCode, oi);
					i += 4;
					oi += 4;
					int off = getAndCopyFourBytesInt(aCode, i, pOutCode, oi);
					i += 4;
					oi += 4;
					// System.out.println( "lookupswitch: idx: " + idx +
					// ", off: " + off);
				}
			...
			case OP_TABLESWITCH: {
				...
				i += 4;
				oi += 4;
				int low = getAndCopyFourBytesInt(aCode, i, pOutCode, oi);
				i += 4;
				oi += 4;
				int hig = getAndCopyFourBytesInt(aCode, i, pOutCode, oi);
				i += 4;
				oi += 4;
				// System.out.println( "tableswitch: dft: " + dft + ", low: " +
				// low + ", hig: " + hig + ", padding: " + (i - oi));
			...
			default:
				...
				if (pArgs == -1) {
					throw new TinyVMException("Bug CU-1: Got " + pOpCode
							+ " in " + iFullName + ".");
				}
	...
	@SuppressWarnings("unused")
	public void processCalls(byte[] aCode, JavaClass aClassFile, Binary aBinary)
			throws TinyVMException {
		int i = 0;
		while (i < aCode.length) {
			...
			case OP_GETSTATIC: {
				int idx = (aCode[i] & 0xFF) << 8 | (aCode[i + 1] & 0xFF);
				if (isWrapperTYPEField(idx)) {
					getStaticFieldClass(idx).getPrimitiveClass()
							.getClassConstant().markUsed();
				} else
					markStaticField((aCode[i] & 0xFF) << 8
							| (aCode[i + 1] & 0xFF));
				i += 2;
				break;
			}
			...
			case OP_LOOKUPSWITCH: {
				...
				int dft = getFourBytesInt(aCode, i);
				i += 4;
				int npairs = getFourBytesInt(aCode, i);
				i += 4;
				// System.out.println( "lookupswitch: dft: " + dft +
				// ", npairs: " + npairs + ", padding: " + (i - oi));
				i += 8 * npairs;
			...
			case OP_TABLESWITCH: {
				...
				int dft = getFourBytesInt(aCode, i);
				i += 4;
				int low = getFourBytesInt(aCode, i);
				i += 4;
				int hig = getFourBytesInt(aCode, i);
				i += 4;
				// System.out.println( "tableswitch: dft: " + dft + ", low: " +
				// low + ", hig: " + hig + ", padding: " + (i - oi));
